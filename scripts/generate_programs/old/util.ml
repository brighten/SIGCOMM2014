...
(* Creates an array which contains k random elements of a (or fewer if a is smaller).
 * The order will also be randomized. *)
let random_subarr a k =
	if k >= Array.length a then
		permute_arr (Array.copy a)
	else (
...
let filter lst f =         List.filter  f lst
...
(* Load a tab separated value file (i.e., a matrix with columns separated by tabs, rows separated by line).
 * Returns a list, each element being a row (a line in the file) represented as an array.
 * List guaranteed to be in same order as file.  All elements of the array are strings.
 *)
let load_tsv path =
	let infile = open_in path in
	let db = ref [] in
	(try
		while true do
			...
			push db (Array.of_list (Str.split_delim (Str.regexp "\t") (line)))
		done
	with End_of_file -> ());
	List.rev !db

(* Same as above, but the first line is treated as column names.  Returns a pair:
 *  - the big matrix, same format as above, but without the first line
 *  - a function get_cell : col_name -> row -> cell
 *)
let load_tsv_with_column_names path =
	let everything = load_tsv path in
	let cols       = List.hd everything in
	let db         = List.tl everything in
	let colhash    = Hashtbl.create 1 in
	for i = 0 to Array.length cols - 1 do
		Hashtbl.add colhash cols.(i) i
		done;
	let get_cell col_name row =
		row.(Hashtbl.find colhash col_name)
		in
	(db, get_cell)
...
(* Compute the mean deviation of a list of floats (average distance from mean) *)
let mean_deviation data =
	let n = i2f (List.length data) in
	let mean = (sum_floats data) /. n in
	let sum_deviation = ref 0.0 in
	foreach data (fun x ->
		sum_deviation := (abs_float (x -. mean)) +. !sum_deviation
		);
	!sum_deviation /. n
...
(* A string to format gnuplot nicely *)
let beautiful_gnuplot_config = "
...
let stats ?(num_trials: int          = 1)
	...
	(* Write raw data file *)
	let var2domain = List.rev var2domain in (* we process it in reverse later, so... *)
...
*)
			let num_trials =
				...
				in
			for i = 1 to num_trials do
				(* Get a data point (for each statistic).  Reverse since we process in reverse order later. *)
				let names_and_samples = List.rev (f var1val var2val (i-1)) in
				let names, samples = List.split names_and_samples in
...
%s%s
plot [] [] \\\n"
			base_plot_name beautiful_gnuplot_config x_name y_name
		...
		foreach_i var2domain (fun j (var2valname, var2val) ->
			...
			(* NEW VERSION: use line style defined in my beautiful gnuplot config *)
			let linetype = sprintf "ls %d" (j+1) in
